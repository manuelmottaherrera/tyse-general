...
export const useRecaptcha = (action?: string) => {
  ...
  const verifyRecaptcha = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (!tokenGenerated) {
        setError('reCaptcha aún no generado, intente más tarde.');
        return false;
      }
      const response = await recaptchaService.verifyToken(tokenGenerated, action || 'default');
      if (!response.success) {
        setError('reCaptcha no válido, intente más tarde.');
        return false;
      }
      setError(null);
      return true;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Error de reCAPTCHA';
